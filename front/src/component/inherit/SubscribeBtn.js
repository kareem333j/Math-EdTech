...
export const SubscribeBtn = (props) => {
        ...
        return (
            <>
                <button className="subscribe-btn w-100 text-center is_subscribed d-flex gap-2 justify-content-center align-items-center" onClick={props.onClick} style={{ 'backgroundColor': props.bgColor, 'color': props.color }}>
                    <AccessTimeIcon />
                    {props.subscribe?<LockOpenIcon sx={{marginBottom:'3px'}} />:null} {props.name} 
                </button>
            </>
        )
    }
    return (
        <>
            <Link className="subscribe-btn w-100 text-center" onClick={props.onClick} to={props.to} style={{ 'backgroundColor': props.bgColor, 'color': props.color }}>{props.subscribe?<LockOpenIcon sx={{marginBottom:'3px'}} />:null} {props.name}</Link>
        </>
    )
}