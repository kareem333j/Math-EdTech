...
class QuizChoice(models.Model):
    choice = models.TextField(blank=True, null=True, default="")
...
def generate_unique_transaction_number(model, field_name="transaction", length=10):
    chars = string.ascii_uppercase + string.digits 
    while True:
        transaction = ''.join(random.choices(chars, k=length))
        if not model.objects.filter(**{field_name: transaction}).exists():
            return transaction
...
class UserCourse(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.transaction:
            self.transaction = generate_unique_transaction_number(UserCourse)
