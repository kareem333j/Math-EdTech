...
class SearchPayments(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if item == "null":
            ...
            queryset = UserCourse.objects.annotate(
                full_name=Concat("user__first_name", Value(" "), "user__last_name")
            ).filter(
                Q(transaction__contains=item)
                | Q(full_name__contains=item)
                | Q(user__phone__contains=item)
            )
...
class BulkActivatePayments(APIView):
    permission_classes = [AdminAndStaffCustomPermissions]
    ...
    def put(self, request, *args, **kwargs):
        ...
        request.data:
        {
            "ids": [1, 2, 3],    # أو تقدر تبعتها بالtransactions
            "action": "activate" # أو "deactivate"
        }
        """
        ...
        payments = UserCourse.objects.filter(id__in=ids)
        print("Found payments:", payments)

        if not ids or action not in ["activate", "deactivate"]:
            return Response({"detail": "بيانات غير صحيحة"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if not payments.exists():
            return Response({"detail": "لم يتم العثور على عمليات"}, status=status.HTTP_404_NOT_FOUND)

        # تحديد القيمة الجديدة بناءً على الـ action
        new_state = True if action == "activate" else False

        updated_count = 0
        for payment in payments:
            payment.is_active = new_state
            payment.save()
            send_activation_message(payment.user, payment.course, new_state)
            updated_count += 1

        return Response(
            {"detail": f"تم تعديل {updated_count} عملية بنجاح", "new_state": new_state},
